...
import {
  getTokenRequest, getUserRequest
...
export const OrderItemPage = () => {
  ...
  const orders = (feedOrders) =>feedOrders?feedOrders.find(item => item._id === id):null
        ...
        <div className={OIPStyles.wrap}>
          ...
          <>
            <section className={`${OIPStyles.header} mb-15`}>
              ...
              <span className={`text text_type_main-small ${OIPStyles.status}` }>
                      {getStatus(order.status)}</span>
            </section>
            <h2 className={`text text_type_main-medium mb-6 ${OIPStyles.composition}`}>Состав:</h2>
              ...
              <ul className={`${OIPStyles.ingredients_list}`}>
                {newIngredient.newIngredients.map((item,i) => (
                  <li key={i} className={OIPStyles.ingredient_item}>
                    <img className={OIPStyles.ingredient_img} alt={'ингредиент'}
                        src={item.ingredient.image}/>
                    <h3
                      className={`${OIPStyles.ingredient_title} text text_type_main-default`}>{item.ingredient.name}</h3>
                    <span
                      className={`${OIPStyles.count} text_type_digits-default`}>{item.count} X {item.ingredient.price}
            ...
            <section className={OIPStyles.footer}>
              <time
                // @ts-ignore
                className={`${OIPStyles.footer_date} text text_color_inactive text_type_main-default`}>{getDate(order.createdAt)}
              </time>
